      ...
      (case (car m)
	    ...
	    (t
	     (let ((f (assoc (car m) *interpfunc* :test #'equal)))
	       (if f
		   (let ((m2 (apply (cdr f) (cdr m))))
		     (interptrm m2 unitvars))
...
(setq *funvar* (make-hash-table :test #'equal))
      ...
      (case (car m)
	    ...
	    (t
		 ...
		 (let* ((s (mapcar #'(lambda (n) (interptrm-th0 n unitvars vars)) (cdr m)))
		   ...
		   (if (equal s2 "")
		       (if (gethash (car m) *funvar*)
			   (format nil "Z~d" (car m))
			 (format nil "c~d" (car m)))
		     (if (gethash (car m) *funvar*)
			 (format nil "(Z~d~d)" (car m) s2)
...
(defun iteify-args (xl)
  (if xl
      (if (cdr xl)
	  (list 'ITE 'B (car xl) (iteify-args (cdr xl)))
	(car xl))
    (throw 'skip-due-to-noargs nil)))
      
(defun argify-vars (xil xbl m)
  (cond ((consp m)
	 (cons (car m) (mapcar #'(lambda (n) (argify-vars xil xbl n)) (cdr m))))
	((eq m 'X) (iteify-args xil))
	((eq m 'Y) (iteify-args xbl))
	(t m)))
...
(defun closedsp (ml)
  (if ml
      (and (closedp (car ml)) (closedsp (cdr ml)))
    t))

(defun get-vert-features-n (n m)
  (if (> n 0)
      (cond ((consp m)
	     (case (car m)
		   (NNEG
		    (let ((fl (get-vert-features-n (- n 1) (cadr m))))
		      (mapcar #'(lambda (x) (cons 'NNEG x)) fl)))
		   (EVENP
		    (let ((fl (get-vert-features-n (- n 1) (cadr m))))
		      (mapcar #'(lambda (x) (cons 'EVENP x)) fl)))
		   (HALVE
		    (let ((fl (get-vert-features-n (- n 1) (cadr m))))
		      (mapcar #'(lambda (x) (cons 'HALVE x)) fl)))
		   (DOUBLE
		    (let ((fl (get-vert-features-n (- n 1) (cadr m))))
		      (mapcar #'(lambda (x) (cons 'DOUBLE x)) fl)))
		   (+
		    (let ((fl (get-vert-features-n (- n 1) (caddr m))))
		      (mapcar #'(lambda (x) (cons '+ x)) fl)))
		   (-
		    (let ((fl (get-vert-features-n (- n 1) (caddr m))))
		      (mapcar #'(lambda (x) (cons '- x)) fl)))
		   (<
		   ...
		   (<=
		    (let ((fl (get-vert-features-n (- n 1) (caddr m))))
		      (mapcar #'(lambda (x) (cons '<= x)) fl)))
		   (ite
		    (let ((fl1 (get-vert-features-n (- n 1) (cadr m)))
			  (fl2 (get-vert-features-n (- n 1) (caddr m)))
			  (fl3 (get-vert-features-n (- n 1) (cadddr m))))
		      (append (mapcar #'(lambda (x) (cons 'ite x)) fl1)
			      (mapcar #'(lambda (x) (cons 'ite x)) fl2)
			      (mapcar #'(lambda (x) (cons 'ite x)) fl3))))
		   (t '(nil))))
	    ((member m '(X Y)) '((VAR)))
	    ((member m '(C B)) '((CONST)))
	    (t '(nil)))
...
(defun get-horiz-features-n (m)
  (if (and (consp m) (>= (length m) 2) (consp (cadr m)) (consp (caddr m)))
      (list (list 'HORIZ (car m) (caadr m) (caaddr m)))
    nil))

(defun get-other-features (m)
  (let ((fl nil))
    (if (consp m)
	(when (not (find-if #'consp (cdr m))) (push '(SMALL) fl))
      (push '(SMALL) fl))
    fl))

(defun get-features (m)
  (let ((fl (append (get-features-2 m)
		    (get-other-features m))))
    (remove-duplicates fl :test #'equal)))

(defun get-features-2 (m)
  (let ((fl (append (mapcar #'(lambda (x) (cons 'VERT x)) (remove-if-not #'(lambda (x) (= (length x) 3)) (get-vert-features-n 3 m)))
		    (get-horiz-features-n m))))
    (if (consp m)
	(dolist (m1 (cdr m) (remove-duplicates fl :test #'equal))
	  (setq fl (append (get-features-2 m1) fl)))
      (remove-duplicates fl :test #'equal))))

(setq *negfeatures* nil)
(setq *posfeatures* '((SMALL)))

(defun jaccard-index (features)
  (let ((num (length (intersection features *posfeatures* :test #'equal)))
	(denom (length (union features *posfeatures* :test #'equal))))
    (/ num denom)))

(defun insert1 (beam w m l r)
  (if l
      (if (> (cadar l) w)
	  (if (> beam 0)
	      (insert1 (- beam 1) w m (cdr l) (cons (car l) r))
	    (throw 'beamout 'beamout))
	(append (reverse r) (cons m l)))
    (append (reverse r) (list m))))

(defun insert (beam m l)
  (let ((features (get-features m)))
    (if (intersection features *negfeatures* :test #'equal)
	l
      (let ((w (jaccard-index features)))
	(let ((r (catch 'beamout
		   (insert1 beam w (list m w) l nil))))
	  (if (eq r 'beamout)
	      l
	    r))))))

(defun reduce-to-beam (beam l)
  (if (> (length l) beam)
      (subseq l 0 beam)
    l))

(setq *synthproph* (make-hash-table :test #'equal))

(defun synthprop (n beam)
  (if (gethash (cons n beam) *synthproph*)
      (cdr (gethash (cons n beam) *synthproph*))
    (let ((rl (synthprop-real n beam)))
      (setf (gethash (cons n beam) *synthproph*) (cons t rl))
      rl)))

(defun synthprop-real (n beam)
  (if (> n 0)
      (let ((fl (synthterm (- n 1) beam))
	    (pl (synthprop (- n 1) beam))
	    (rl (insert beam 'B (insert beam 'Y nil))))
	(format t "synthprop-real ~d~%" n) (force-output)
	(dolist (t1 pl)
	  (format t "synthprop-real t1: ~d~%" t1) (force-output)
	  (when (and (consp t1) (consp (car t1)))
	    (unless (member (caar t1) '(FALSE TRUE NOT))
	      (format t "synthprop-real t1 pushing in not: ~d~%" t1) (force-output)
	      (format t "before: ~d~%" rl)
	      (setq rl (insert beam (list 'not (car t1)) rl))
	      (format t "after: ~d~%" rl)
	      )))
	(dolist (t1 fl)
	  (setq rl (insert beam (list 'nneg (car t1)) rl))
	  (setq rl (insert beam (list 'evenp (car t1)) rl))
	  (dolist (t2 fl)
	    (setq rl (insert beam (list '< (car t1) (car t2)) rl))
	    (setq rl (insert beam (list '<= (car t1) (car t2)) rl))))
	(format t "synthprop-real end ~d~%" n) (force-output)
	(format t "before r2b: ~d~%" rl) (force-output)
	(setq rl (reduce-to-beam beam rl))
	(format t "after r2b: ~d~%" rl) (force-output)
	rl)
    nil))

(setq *synthtermh* (make-hash-table :test #'equal))

(defun synthterm (n beam)
  (if (gethash (cons n beam) *synthtermh*)
      (cdr (gethash (cons n beam) *synthtermh*))
    (let ((rl (synthterm-real n beam)))
      (setf (gethash (cons n beam) *synthtermh*) (cons t rl))
      rl)))

(defun synthterm-real (n beam)
  (if (> n 0)
      (let ((fl (synthterm (- n 1) beam))
	    (pl (synthprop (- n 1) beam))
	    (rl (insert beam 'x (insert beam 'c nil))))
	(dolist (t1 fl (reduce-to-beam beam rl))
	  (unless (eq (car t1) 'C) ; to prevent pointless ones
	    (setq rl (insert beam (list 'double (car t1)) rl))
	    (setq rl (insert beam (list 'halve (car t1)) rl))
	    (setq rl (insert beam (list '+ 'c (car t1)) rl))
	    (setq rl (insert beam (list '- 'c (car t1)) rl)))
	  (dolist (p pl)
	    (unless (or (closedp (car p))
			(and (consp (car p))
			     (eq (caar p) 'NOT)))
	      (dolist (t2 fl)
		(unless (and (equal (car t1) (car t2)) (not (free-in 'C (car t1))))
		  (setq rl (insert beam (list 'ite (car p) (car t1) (car t2)) rl))))))))
    (insert beam 'x (insert beam 'c nil))))
...
(defun split-consts-and-quote (xl m)
  (if (eq m 'C)
	...
	(let ((c (read-from-string (format nil "B~d" *bc*))))
	  ...
	  (if (eq (car m) 'VAR)
	      (cadr m)
	    (cons 'list (cons (list 'quote (car m)) (mapcar #'(lambda (n) (split-consts-and-quote xl n)) (cdr m)))))
	(if (member m xl)
	    m
	  (list 'quote m))))))

(defun split-consts-and-dquote (xl m)
  (if (eq m 'C)
      (let ((c (read-from-string (format nil "C~d" *cc*))))
	(incf *cc*)
	(push c *ccl*)
...
; $o is bool (= prop), $i is int
(defun output-interp-th0-sat-real (g ll sortl funl al)
  (dolist (x sortl)
    (unless (or (assoc x *interptp* :test #'equal) (member x *unittp* :test #'equal))
      (throw 'fail '(fail "a sort was uninterpreted in output-interp-th0-sat-real"))))
  (when (or *bcl* *ccl*) (throw 'fail '(fail "bcl or ccl was nonempty in output-interp-th0-sat-real")))
  (setq *funvar* (make-hash-table :test #'equal))
  (format g "thf(conj,conjecture,")
  (let ((ecnt 0))
    (dolist (x funl)
      (unless (gethash (car x) *ret-unit*)
	(unless (or (assoc (car x) *interpfunc* :test #'equal)
    ...
    (dolist (m al)
      (unless (trivial-after-interp m)
	(catch 'th0-fail
	  (format g " & ~d" (interptrm-th0 m)))))
    (format g ")")
    (dotimes (i ecnt) (format g ")"))
    (format g ").~%")))

(defun output-interp-th0-sat (g ll sortl funl al)
  (output-interp-th0-sat-real g ll sortl funl al)
  (when (> *verbosity* 99) (format t "====~%") (output-interp-th0-sat-real *standard-output* ll sortl funl al) (format t "====~%")))
...
(defun ite-tree (xl r)
  (if xl
      (let ((m (ite-tree (cdr xl) r)))
	(list 'ite (list 'VAR (car xl)) m m))
    r))
...
(defun smtintfuncsynth-2 (sortposs fn ll sortl funl al)
  ...
  (let ((g (open (format nil "~d.tmp.th0.p" fn) :direction :output :if-exists :supersede :if-does-not-exist :create)))
    (output-interp-th0-sat g ll sortl funl al)
    (close g)
    (break)))
...
(defun smtintfuncsynth-th0int (fn)
  (let ((tm (get-universal-time)))
    (let ((r (catch 'sat (smtintfuncsynth-th0int-real fn))))
      (if (and (consp r) (eq (car r) 'SAT))
...
(defun smtintfuncsynth-th0int-real (fn)
  (multiple-value-bind
   (ll sortl funl al)
   (readsmt fn)
   (setq *infsort* (make-hash-table :test #'equal))
